...
export function nextDate(now, index = 0) {
    throwIfInvalidDate(now);
    const date = new Date(now.getFullYear(), now.getMonth(), now.getDate() + index, now.getHours(), now.getMinutes(), now.getSeconds());
    return date;
}

export function nextHour(now, index = 0) {
    throwIfInvalidDate(now);
    const hour = now.getHours() + index
    const date = new Date(now.getFullYear(), now.getMonth(), now.getDate(), hour, now.getMinutes(), now.getSeconds());
    return date;
}

export function nextMinite(now, index = 0) {
    throwIfInvalidDate(now);
    const minites = now.getMinutes() + index;
    const date = new Date(now.getFullYear(), now.getMonth(), now.getDate(), now.getHours(), minites, now.getSeconds());
...
export function nextSecond(now, index = 0) {
    throwIfInvalidDate(now);
    const second = now.getSeconds() + index;
    const date = new Date(now.getFullYear(), now.getMonth(), now.getDate() + index, now.getHours(), now.getMinutes(), second);
    return date;
}
